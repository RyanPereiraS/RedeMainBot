...
const BanChannel = "806699440756228169"
module.exports.run = async (client, message, args) => {
    let error;

    if(!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send('Você não tem permissão bobinho.');
    let user = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
    if(!user) return falha("Nenhum usuário mencionado");
    ...
    function BanChannelMessage() {
        const BanMessage = new Discord.MessageEmbed()
            .setTitle(`<:624603272958705664:806677999058550845> Banido <:624603272958705664:806677999058550845>`)
            ...
            .addField("<:767835965028565002:806677999122120714> Banido Por: ", message.author)
            .setFooter(`${message.author.tag}`, message.author.avatarURL())
            .setColor("RANDOM").setTimestamp()
            client.channels.cache.get(BanChannel).send(BanMessage).catch(err => {
                error = err;
                falha(err)
            })
    }
